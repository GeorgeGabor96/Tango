...
input_spike_matrix_create(Memory* memory, u32 time_max, u32 n_neurons) {
    ...
    InputSpikeMatrix* matrix = (InputSpikeMatrix*)memory_push_zero(memory, sizeof(InputSpikeMatrix) + sizeof(b8) * time_max * n_neurons);
    check_memory(matrix);
    matrix->time = time_max;
    matrix->n_neurons = n_neurons;
    matrix->data = (b8*)(matrix + 1);
    return matrix;
...
input_spike_matrix_get_spike_data_for_time(InputSpikeMatrix* matrix, u32 time) {
    ...
    b8* result = matrix->data + time * matrix->n_neurons;
    return result;
...
input_spike_matrix_create_from_spike_times(Memory* memory, InputSpikeTimes* spike_times) {
    ...
    check(spike_times->type == INPUT_SPIKE_TIMES_IMAGE,
        ...
        input_spike_times_get_c_str(spike_times->type),
        input_spike_times_get_c_str(INPUT_SPIKE_TIMES_IMAGE));

    InputSpikeMatrix* matrix = input_spike_matrix_create(memory, 255,
