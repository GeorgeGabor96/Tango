...
int main() {
    Experiment* exp = experiment_create(4, 723104, "D:\\repos\\Tango_outputs\\experimenting\\scale_10_exp_100_steps");

    Memory* memory = exp->permanent_memory;

    DataGen* spike_train_data = data_gen_create_spike_train(memory, 300, "d:/datasets/MNIST/encoding_10_imgs", "d:/datasets/MNIST/encoding_10_imgs/samples.txt", 230, 200);
    ...
    synapse_cls_add_learning_rule_exponential(s_cls_exci, 0.0f, 2.0f, 0.1f, -1.0f, 50);
    ...
    Callback* cb_weights = callback_weights_dumper_create(
        memory, 1, 200, exp->output_folder, net);
