...
int main() {
    Experiment* exp = experiment_create(0, 723104, "D:\\repos\\Tango_outputs\\exp\\mnist_roc_if_one_spike");

    Memory* memory = exp->permanent_memory;

    DataGen* spike_train_data = data_gen_create_spike_train(memory, 500, "d:/datasets/MNIST/encoding/roc/img", "d:/datasets/MNIST/encoding/roc/img/samples.txt", 200, 100);
